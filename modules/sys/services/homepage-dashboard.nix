...
let
  ...
  auto_columns =
    name:
    lib.length (
      lib.elemAt (lib.attrValues (
        lib.findFirst (item: (lib.attrNames item) == [ name ]) { "" = [ ]; } cfg.services
      )) 0
    );
in
{
  options.my.homepage-dashboard.enable = mkEnableOption "enable homepage-dashboard" // {
    default = config.is_client;
  };
  config = mkIf config.my.homepage-dashboard.enable {
    ...
    services.homepage-dashboard = {
      ...
      services =
        let
          tasks_md = config.services.tasks_md;
        in
        (lib.optional tasks_md.enable {
          "Tasks" = map (item: {
            "${item.title}" = rec {
