...
in
{
  options.my.nix.distributed-builds.enable = mkEnableOption "configure distributed builds" // {
    default = (
      !config.is_microvm
      && config.services.wireguard-wrapper.enable
      && config.networking.hostName != "deus"
      && config.networking.hostName != "dea"
    );
  };
  config = mkIf config.my.nix.distributed-builds.enable {
    nix.buildMachines = [
      {
