{ pkgs, config, lib, ... }:
let hostnames = [ "cups.localhost" "print.localhost" ];
in {
  services.printing = {
  ...
  in lib.mkIf config.services.nginx.enable (mapListToAttrs (d: {
    name = d;
    value = {
      locations."/" = {
        proxyPass =
          "https://${lib.head config.services.printing.listenAddresses}";
        extraConfig = ''
