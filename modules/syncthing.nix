...
let
  ...
  servers = { syncschlawiner = { id = ids.syncschlawiner; }; };
  all_servers = { tabula = { id = ids.tabula; }; };
  folders_list = with all_handys all_pcs servers;
    (create_folder "3d_printing" (all_pcs // servers)
      ...
      // create_folder "Downloads" (all_pcs // servers) // create_folder_adv {
        name = "Family";
      ...
      } // create_folder "Music" (all_pcs // servers) // create_folder_adv {
        name = "Passwords";
      ...
      // create_folder "AegisBak"
      (all_pcs // servers // { inherit handyHannes; })
      // create_folder "SignalBackup"
      (all_pcs // servers // { inherit handyHannes; })
      // create_folder "DownloadHandyH"
      (all_pcs // servers // { inherit handyHannes; })
      // create_folder "HannesKamera"
      (all_pcs // servers // { inherit handyHannes; })
      // create_folder "HannesGalerie"
      (all_pcs // servers // { inherit handyHannes; })
      // create_folder "AlexandraKamera"
      (servers // { inherit handyAlexandra; })
      // create_folder "AlexandraGalerie"
      (servers // { inherit handyAlexandra; })
      // create_folder "ThomasKamera" (servers)
      // create_folder "ThomasGalerie" (servers)
      // create_folder "website" (all_pcs // { inherit tabula; }));
  create_folder_adv = { name, devices, versioning ? default_versioning
    , paths ? { }
    , default_path ? config.services.syncthing_wrap.dataDir + "/" + name }: {
      "${name}" = {
        inherit versioning;
        path =
          (if paths ? "${dev_name}" then paths."${dev_name}" else default_path);
        devices = (builtins.attrNames devices);
      };
    };
  create_folder = name: devices:
    create_folder_adv { inherit name devices versioning; };
