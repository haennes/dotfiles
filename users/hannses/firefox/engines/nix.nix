...
lib.mapAttrs (_name: value: if value ? icon then value else value // { inherit icon; }) {
  ...
  "nixvim" = {
    urls = [
      {
        template = "https://nix-community.github.io/nixvim/plugins/nix.html?search={searchTerms}";
      }
    ];
  ...
  "github nixpkgs issues" = {
    urls = [
      {
        template = "https://github.com/search?q=repo%3ANixOS%2Fnixpkgs+{searchTerms}&type=issues";
      }
    ];
