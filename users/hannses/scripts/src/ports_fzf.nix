...
let
  ...
  curr_ssh_ports_files = mapAttrs (
    n: v: pkgs.writeText "ports-ssh-${n}" (mapPortsToString v)
  ) curr_ssh_ports;
  curr_ssh_ports_file_toplevel =
    let
      list = mapAttrsToList (n: v: "ln -s ${v} ${n}") curr_ssh_ports_files;
    in
    pkgs.runCommand "ports-ssh" { } ''
      mkdir -p $out
      cd $out
      ${concatLines list}
    '';
