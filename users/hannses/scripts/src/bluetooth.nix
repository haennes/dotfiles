{ pkgs, globals, ... }:  ''
    ...
    # Checks if a device is connected
        ...
        if echo -e "$device_info" | grep -F -q "Connected: yes"; then
            return 0
        else
            return 1
        fi
    ...
    # Checks if a device is paired
        device_paired() {
        device_info=$(bluetoothctl info "$1")
        if echo -e "$device_info" | grep -F -q "Paired: yes"; then
            echo "Paired: yes"
            return 0
        else
            echo "Paired: no"
    ...
    # Checks if a device is trusted
        device_trusted() {
        device_info=$(bluetoothctl info "$1")
        if echo -e "$device_info" | grep -F -q "Trusted: yes"; then
            echo "Trusted: yes"
            return 0
        else
            echo "Trusted: no"
    ...
    # A submenu for a specific device that allows connecting, pairing, and trusting
        ...
        device_name=$(echo -e $device | cut -d ' ' -f 3-)
        mac=$(echo -e $device | cut -d ' ' -f 2)
    ...
    # Opens a dmenu menu with current bluetooth status and options to connect
        ...
        # Open dmenu menu, read chosen option
        chosen="$(echo -e "$options" |  $DMENU "Bluetooth")"
