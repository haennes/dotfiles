...
let
  ...
  dnatRuleMode =
    ...
    if rule.mode != "" then
      ...
      rule.target4.address or null == netAddresses.lan4
      || rule.target6.address or null == netAddresses.lan6
    then
      "rule"
    else
      "mark";
  ...
  dnatRuleProtos =
    ...
    else if rule.tcp then
      tcp
    else if rule.udp then
      udp
    else
      throw "Invalid rule: either tcp or udp must be set";

  setIfNeeded = arr: if builtins.length arr == 1 then builtins.head arr else notnft.dsl.set arr;
      ...
      filter = add table.netdev (
        {
          ingress_common =
            add chain
              (
                [ (is.eq (bit.and tcp.flags (f: bit.or f.fin f.syn)) (f: bit.or f.fin f.syn)) ]
                ++ logIfWan "${logPrefix}fin+syn drop "
                ++ [ drop ]
              )
              (
                [ (is.eq (bit.and tcp.flags (f: bit.or f.syn f.rst)) (f: bit.or f.syn f.rst)) ]
                ++ logIfWan "${logPrefix}syn+rst drop "
                ++ [ drop ]
              )
              [
                (is.eq (bit.and tcp.flags (f: with f; bit.or fin syn rst psh ack urg)) 0)
                (log "${logPrefix}null drop ")
                drop
              ]
              [
                (is tcp.flags (f: f.syn))
                (is.eq tcpOpt.maxseg.size (range 0 500))
                (log "${logPrefix}maxseg drop ")
                drop
              ]
              # reject requests with own saddr
              # log if they are meant for us...
              [
                (is.eq ip.saddr selfIp4)
                (is.eq (fib (
                  f: with f; [
                    daddr
                    iif
          ...
          ingress_wan_common =
            add chain netdevIngressWanRules [ (jump "ingress_common") ]
              ...
              [
                (is.eq ip.protocol (f: f.icmp))
                (is.ne icmp.type (f: f.echo-request))
                (limit {
                  rate = 100;
                  per = f: f.second;
                })
              ...
              [
                ...
                (is.eq icmpv6.type (f: f.echo-request))
                (limit {
                  rate = 50;
                  per = f: f.second;
                })
              ...
              [
                (is.eq ip6.nexthdr (f: f.ipv6-icmp))
                (is.ne icmpv6.type (f: f.echo-request))
                (limit {
                  rate = 100;
                  per = f: f.second;
                })
              ...
              [
                (is.eq ip.protocol (f: f.icmp))
                (log "${logPrefix}icmp flood ")
                drop
              ]
              [
                (is.eq ip6.nexthdr (f: f.ipv6-icmp))
                (log "${logPrefix}icmp6 flood ")
                drop
              ];
        }
        // extraNetdevEntries
      ...
      global = add table { family = f: f.inet; } (
        {
          inbound_wan_common =
            add chain
              ...
              [
                (is ct.status (f: f.dnat))
                accept
              ]
              (
                [
                  (is.eq (bit.and tcp.flags (f: f.syn)) 0)
                  (is.eq ct.state (f: f.new))
                ]
                ++ logIfWan "${logPrefix}new non-syn "
                ++ [ drop ]
              )
              ...
              [
                ...
                (is.eq icmpv6.code (f: f.no-route))
                (is.eq icmpv6.type (
                  f:
                  with f;
                  set [
              ...
              [
                (is.eq ip6.nexthdr (f: f.ipv6-icmp))
                (is.eq icmpv6.code (f: f.admin-prohibited))
                (is.eq icmpv6.type (f: f.parameter-problem))
                accept
              ]
              inetInboundWanRules;

          # trust the lan
          inbound_lan_common = add chain [ accept ];
          ...
          forward =
            add chain
              ...
              [
                (is.eq meta.iifname (setIfNeeded wans))
                (is.eq icmpv6.type (
                  f:
                  with f;
                  set [
          ...
          snat =
            add chain
              ...
              [
                ...
                (is.eq meta.iifname (setIfNeeded lans))
                (is.eq meta.oifname (setIfNeeded wans))
                masquerade
              ]
              inetSnatRules;
  ...
  netCidrs = builtins.mapAttrs (
    _: v: router-lib.serializeCidr (router-lib.normalizeCidr v)
  ) netParsedCidrs;
  ...
  hosted-domains = builtins.filter (domain: domain != "localhost") (
    builtins.concatLists (
      builtins.attrValues (
        builtins.mapAttrs (k: v: [ k ] ++ v.serverAliases) server-config.services.nginx.virtualHosts
      )
    )
  );
in
{
  ...
  router-settings.dnatRules =
    let
      ...
      inherit (server-config.networking.firewall)
        allowedTCPPorts
      ...
      tcpAndUdp = builtins.filter (
        x: !builtins.elem x bannedPorts && builtins.elem x allowedTCPPorts
      ) allowedUDPPorts;
    ...
    in
    lib.optional (tcpAndUdp != [ ]) {
      port = setIfNeeded tcpAndUdp;
      tcp = true;
      udp = true;
    ...
    ++ lib.flip map rangesTcpAndUdp (range: {
      port = notnft.dsl.range range.from range.to;
      tcp = true;
      udp = true;
      target4.address = serverAddress4;
      target6.address = serverAddress6;
    ...
    ++ lib.toList {
      ...
      target4.port = 22;
      target6.port = 22;
      target4.address = serverInitrdAddress4;
  ...
  */
  ...
  router.networkNamespaces.default = {
    ...
    ++ builtins.concatLists (
      map (
        ...
        let
          ...
          forEachPort =
            ...
            if builtins.isInt port then
              ...
              builtins.concatLists (map (forEachPort func) port.set)
            else if port ? range.min then
              let
                inherit (port.range) min max;
              in
              [ (func "${toString min}-${toString max}") ]
            else if port ? range then
              let
                max = builtins.elemAt port.range 1;
                min = builtins.head port.range;
              in
              [ (func "${toString min}-${toString max}") ]
            else
              throw "Unsupported expr: ${builtins.toJSON port}";
          gen =
            len: proto: tgt:
            forEachPort (port: [
              "from"
              "${tgt.address}/${toString len}"
              "ipproto"
              proto
              "sport"
              port
        ...
        in
        lib.optionals (rule.tcp && rule.target4 != null) (
          map (x: {
            ipv6 = false;
            extraArgs = x;
          }) (gen 32 "tcp" rule.target4)
        )
        ++ lib.optionals (rule.udp && rule.target4 != null) (
          map (x: {
            ipv6 = false;
            extraArgs = x;
          }) (gen 32 "udp" rule.target4)
        )
        ++ lib.optionals (rule.tcp && rule.target6 != null) (
          map (x: {
            ipv6 = true;
            extraArgs = x;
          }) (gen 128 "tcp" rule.target6)
        )
        ++ lib.optionals (rule.udp && rule.target6 != null) (
          map (x: {
            ipv6 = true;
            extraArgs = x;
          }) (gen 128 "udp" rule.target6)
        )
    ...
    nftables.jsonRules = mkRules {
      ...
      inetDnatRules =
        ...
        ++ (
          ...
          [
            ...
            [
              (is.eq meta.iifname "br0")
              (is.eq ip.daddr "162.159.36.2")
              (is.eq ip.protocol (
                f:
                set [
                  f.tcp
              ...
              (dnat.ip netAddresses.lan4)
            ]
          ]
          ++ lib.optionals (cfg.naughtyMacs != [ ]) [
            [
              (is.eq meta.iifname "br0")
              (is.eq ether.saddr (setIfNeeded cfg.naughtyMacs))
              (is.eq ip.protocol (
                f:
                set [
                  f.tcp
      ...
      inetForwardRules =
        ...
        [
          ...
          [
            (is.eq meta.iifname "veth-wan-a")
            (is.eq meta.oifname "br0")
            accept
          ]
      ...
      inetInboundWanRules =
        ...
        [
          [
            (is.eq ip.saddr (cidr netCidrs.netns4))
            accept
          ]
          [
            (is.eq ip6.saddr (cidr netCidrs.netns6))
            accept
          ]
        ];
      extraInetEntries =
        ...
        with payload;
        {
          ...
          force_unvpn4 = add set {
            type = f: f.ipv4_addr;
            flags = f: with f; [ interval ];
          };
          force_unvpn6 = add set {
            type = f: f.ipv6_addr;
            flags = f: with f; [ interval ];
          };
          force_vpn4 = add set {
            ...
            add chain
              ...
              (
                [
                  ...
                  [
                    (is.eq ip.daddr "@block4")
                    (log "block4 ")
                    drop
                  ]
                  [
                    (is.eq ip6.daddr "@block6")
                    (log "block6 ")
                    drop
                  ]
                  [
                    (is.eq ip.saddr "@block4")
                    (log "block4/s ")
                    drop
                  ]
                  [
                    (is.eq ip6.saddr "@block6")
                    (log "block6/s ")
                    drop
                  ]
                  # default to vpn...
                  ...
                  [
                    (is.eq ip.daddr "@force_unvpn4")
                    (mangle meta.mark wan_table)
                  ]
                  [
                    (is.eq ip6.daddr "@force_unvpn6")
                    (mangle meta.mark wan_table)
                  ]
                  [
                    (is.eq ip.saddr "@force_unvpn4")
                    (mangle meta.mark wan_table)
                  ]
                  [
                    (is.eq ip6.saddr "@force_unvpn6")
                    (mangle meta.mark wan_table)
                  ]
                  ...
                  [
                    (is.eq meta.iifname "br0")
                    (is.ne ether.saddr cfg.serverMac)
                    (is.eq meta.l4proto (f: f.tcp))
                    (is.eq tcp.dport 25)
                    (log "smtp ")
                    drop
                  ]
                  ...
                  map (
                    ...
                    lib.optionals (rule4 != null) [
                      [
                        (is.eq meta.iifname "br0")
                        (is.eq ip.protocol protocols)
                        (is.eq ip.saddr rule4.address)
                        (is.eq th.sport (if rule4.port != null then rule4.port else rule.port))
                        (mangle meta.mark wan_table)
                      ]
                      [
                        (is.eq meta.iifname "veth-wan-a")
                        (is.eq ip.protocol protocols)
                        (is.eq ip.daddr rule4.address)
                        (is.eq th.dport (if rule4.port != null then rule4.port else rule.port))
                        (mangle meta.mark wan_table)
                      ]
                    ]
                    ++ lib.optionals (rule6 != null) [
                      [
                        (is.eq meta.iifname "br0")
                        (is.eq ip6.nexthdr protocols)
                        (is.eq ip6.saddr rule6.address)
                        (is.eq th.sport (if rule6.port != null then rule6.port else rule.port))
                        (mangle meta.mark wan_table)
                      ]
                      [
                        (is.eq meta.iifname "veth-wan-a")
                        (is.eq ip6.nexthdr protocols)
                        (is.eq ip6.daddr rule6.address)
                        (is.eq th.dport (if rule6.port != null then rule6.port else rule.port))
                        (mangle meta.mark wan_table)
                      ]
                    ]
                ...
                ++ [
                  ...
                  [
                    ...
                    (is.ne ip.daddr (cidr netCidrs.lan4))
                    (is.ne ip.daddr "@allow_iot4")
                    (log "iot4 ")
                    drop
                  ]
                  [
                    (is.eq ether.saddr cfg.vacuumMac)
                    (is.ne ip6.daddr (cidr netCidrs.lan6))
                    (is.ne ip6.daddr "@allow_iot6")
                    (log "iot6 ")
                    drop
                  ]
                  [
                    (is.eq ether.daddr cfg.vacuumMac)
                    (is.ne ip.saddr (cidr netCidrs.lan4))
                    (is.ne ip.saddr "@allow_iot4")
                    (log "iot4/d ")
                    drop
                  ]
                  [
                    (is.eq ether.daddr cfg.vacuumMac)
                    (is.ne ip6.saddr (cidr netCidrs.lan6))
                    (is.ne ip6.saddr "@allow_iot6")
                    (log "iot6/d ")
                    drop
                  ]
                  ...
                  [
                    (is.eq meta.nfproto (f: f.ipv4))
                    (is.eq meta.mark vpn_table)
                    (is.gt tcpOpt.maxseg.size vpn_ipv4_mss)
                    (mangle tcpOpt.maxseg.size vpn_ipv4_mss)
                  ]
                  [
                    (is.eq meta.nfproto (f: f.ipv6))
                    (is.eq meta.mark vpn_table)
                    (is.gt tcpOpt.maxseg.size vpn_ipv6_mss)
                  ...
                  [
                    (is.eq meta.iifname "br0")
                    (is.ne ip.daddr (netAddresses.lan4))
                    (is.eq ip.protocol (
                      f:
                      set [
                        f.tcp
                  ...
                  [
                    (is.eq meta.iifname "br0")
                    (is.ne ip6.daddr (netAddresses.lan6))
                    (is.eq ip6.nexthdr (
                      f:
                      set [
                        f.tcp
  ...
  router.networkNamespaces.wan = {
    ...
    nftables.jsonRules = mkRules {
      ...
      inetDnatRules = builtins.concatLists (
        map (
          rule:
          let
            protocols = dnatRuleProtos rule;
      ...
      inetInboundWanRules =
        ...
        [
          # DHCP
          [
            (is.eq meta.nfproto (x: x.ipv4))
            (is.eq udp.dport 68)
            accept
          ]
          [
            (is.eq meta.nfproto (x: x.ipv6))
            (is.eq udp.dport 546)
            accept
          ]
          # igmp, used for setting up multicast groups
          [
            (is.eq ip.protocol (f: f.igmp))
            accept
          ]
  ...
  services.unbound = {
    ...
    settings = {
      server = rec {
        ...
        access-control = [
          "${netCidrs.netns4} allow"
          "${netCidrs.netns6} allow"
          "${netCidrs.lan4} allow"
          "${netCidrs.lan6} allow"
        ];
      ...
      python.python-script = builtins.toFile "avahi-resolver-v2.py" (
        builtins.readFile ./avahi-resolver-v2.py
      );
  ...
  systemd.services.unbound = lib.mkIf config.services.unbound.enable {
    environment.PYTHONPATH =
      let
        unbound-python = pkgs.python3.withPackages (
          ps: with ps; [
            pydbus
            dnspython
            requests
